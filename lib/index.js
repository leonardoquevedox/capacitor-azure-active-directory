...
const ActiveDirectoryPlugin = () => {
   ...
   */
  ActiveDirectoryPlugin.prototype.login = () => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._openPopup = (urlNavigate, title, popUpWidth, popUpHeight) => {
    try {
      /**
  ...
  ActiveDirectoryPlugin.prototype._handlePopupError = (
    loginCallback,
    resource,
    error,
    errorDesc,
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._loginPopup = (urlNavigate, resource, callback) => {
  ...
  ActiveDirectoryPlugin.prototype._broadcast = (eventName, data) => {
  ...
  ActiveDirectoryPlugin.prototype.loginInProgress = () => {
   ...
   */
  ActiveDirectoryPlugin.prototype._hasResource = (key) => {
   ...
   */
  ActiveDirectoryPlugin.prototype.getCachedToken = (resource) => {
   ...
   */
  ActiveDirectoryPlugin.prototype.getCachedUser = () => {
    if (this._user) {
      return this._user
    }
   ...
   */
  ActiveDirectoryPlugin.prototype.registerCallback = (expectedState, resource, callback) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._renewToken = (resource, callback, responseType) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._renewIdToken = (callback, responseType) => {
    // use iframe to try to renew token
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._urlContainsQueryStringParameter = (name, url) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._urlRemoveQueryStringParameter = (url, name) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._loadFrameTimeout = (urlNavigation, frameName, resource) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._loadFrame = (urlNavigate, frameName) => {
   ...
   * @param {tokenCallback} callback -  The callback provided by the caller. It will be called with token or error.
   */
  ActiveDirectoryPlugin.prototype.acquireToken = (resource, callback) => {
   ...
   * @param {tokenCallback} callback -  The callback provided by the caller. It will be called with token or error.
   */
  ActiveDirectoryPlugin.prototype.acquireTokenPopup = (
    resource,
   ...
   * @param {string}   extraQueryParameters  extraQueryParameters to add to the authentication request
   */
  ActiveDirectoryPlugin.prototype.acquireTokenRedirect = (
    resource,
    extraQueryParameters,
    claims,
  ) => {
   ...
   */
  ActiveDirectoryPlugin.prototype.promptUser = (urlNavigate) => {
    if (urlNavigate) {
   ...
   * Clears cache items.
   */
  ActiveDirectoryPlugin.prototype.clearCache = () => {
   ...
   */
  ActiveDirectoryPlugin.prototype.clearCacheForResource = (resource) => {
    this._saveItem(this.CONSTANTS.STORAGE.STATE_RENEW, '')
    this._saveItem(this.CONSTANTS.STORAGE.ERROR, '')
    this._saveItem(this.CONSTANTS.STORAGE.ERROR_DESCRIPTION, '')
   ...
   */
  ActiveDirectoryPlugin.prototype.logOut = () => {
    this.clearCache()
    this._user = null
    let urlNavigate
  ...
  ActiveDirectoryPlugin.prototype._isEmpty = (str) =>
    typeof str === 'undefined' || !str || 0 === str.length
   ...
   */
  ActiveDirectoryPlugin.prototype.getUser = (callback) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._addHintParameters = (urlNavigate) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._createUser = (idToken) => {
    let user = null
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._getHash = (hash) => {
    if (hash.indexOf('#/') > -1) {
   ...
   */
  ActiveDirectoryPlugin.prototype.isCallback = (hash) => {
   ...
   */
  ActiveDirectoryPlugin.prototype.getLoginError = () => {
   ...
   */
  ActiveDirectoryPlugin.prototype.getRequestInfo = (hash) => {
    hash = this._getHash(hash)
    const parameters = this._deserialize(hash)
    const requestInfo = {
      valid: false,
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._matchNonce = (user) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._matchState = (requestInfo) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._getResourceFromState = (state) => {
    if (state) {
   ...
   */
  ActiveDirectoryPlugin.prototype.saveTokenFromHash = (requestInfo) => {
    this.info(
   ...
   */
  ActiveDirectoryPlugin.prototype.getResourceForEndpoint = (endpoint) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._getHostFromUri = (uri) => {
   ...
   */
  ActiveDirectoryPlugin.prototype.handleWindowCallback = (hash) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._getNavigateUrl = (responseType, resource) => {
    let tenant = 'common'
    if (this.config.tenant) {
      tenant = this.config.tenant
    }
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._extractIdToken = (encodedIdToken) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._base64DecodeStringUrlSafe = (base64IdToken) => {
  ...
  ActiveDirectoryPlugin.prototype._decode = (base64IdToken) => {
  ...
  ActiveDirectoryPlugin.prototype._decodeJwt = (jwtToken) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._convertUrlSafeToRegularBase64EncodedString = (str) =>
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._serialize = (responseType, obj, resource) => {
    const str = []
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._deserialize = (query) => {
    let match
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._decimalToHex = (number) => {
   ...
   */
  /* jshint ignore:start */
  ActiveDirectoryPlugin.prototype._guid = () => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._expiresIn = (expires) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._now = () => Math.round(new Date().getTime() / 1000.0)

  /**
   * Adds the hidden iframe for silent token renewal
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._addAdalFrame = (iframeId) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._saveItem = (key, obj, preserve) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._getItem = (key) => {
    if (this.config && this.config.cacheLocation && this.config.cacheLocation === 'localStorage') {
      if (!this._supportsLocalStorage()) {
        this.info('Local storage is not supported')
        return null
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._supportsLocalStorage = () => {
    try {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._supportsSessionStorage = () => {
    try {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._cloneConfig = (obj) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._addLibMetadata = () => {
    // x-client-SKU
    // x-client-Ver
   ...
   */
  ActiveDirectoryPlugin.prototype.log = (level, message, error, containsPii) => {
   ...
   * @param {string} error  -  Error to log.
   */
  ActiveDirectoryPlugin.prototype.error = (message, error) => {
   ...
   * @param {string} message  -  Message to log.
   */
  ActiveDirectoryPlugin.prototype.warn = (message) => {
   ...
   * @param {string} message  -  Message to log.
   */
  ActiveDirectoryPlugin.prototype.info = (message) => {
   ...
   * @param {string} message  -  Message to log.
   */
  ActiveDirectoryPlugin.prototype.verbose = (message) => {
   ...
   * @param {string} error  -  Error to log.
   */
  ActiveDirectoryPlugin.prototype.errorPii = (message, error) => {
   ...
   * @param {string} message  -  Message to log.
   */
  ActiveDirectoryPlugin.prototype.warnPii = (message) => {
   ...
   * @param {string} message  -  Message to log.
   */
  ActiveDirectoryPlugin.prototype.infoPii = (message) => {
   ...
   * @param {string} message  -  Message to log.
   */
  ActiveDirectoryPlugin.prototype.verbosePii = (message) => {
   ...
   * @ignore
   */
  ActiveDirectoryPlugin.prototype._libVersion = () => '1.0.17'
  ...
  if (typeof module !== 'undefined' && module.exports) {
    ...
    module.exports.inject = (conf) => new ActiveDirectoryPlugin(conf)
  }
